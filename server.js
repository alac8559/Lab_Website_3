...
app.get('/team_stats', function(req, res) {
	...
	db.task('my_team_stats', task=> {
		return task.batch([
			task.any(query), task.any(query2), task.any(query3),
		]);
	})
	.then(results=> {
...
app.get('/player_info/post', function(req, res) {
	...
	.then(results=> {
		res.render('pages/player_info', {
			my_title: "Player Info", 
			players: results[0],
			player_actual: results[1],
			games_played: results[2]
		})
	})
